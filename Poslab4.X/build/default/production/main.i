...
PSECT CODE, delta=2, abs
 ...
 bsf TRISB,0
 bsf TRISB,1;Se configuran ((PORTB) and 07Fh), 0 y ((PORTB) and 07Fh), 1 como entradas
 ...
 bsf ((OSCCON) and 07Fh), 0; Se elige el reloj interno del PIC
    return

    config_banderas:
 bsf ((INTCON) and 07Fh), 7; Se habilita el Global Interruption Enable
 bsf ((INTCON) and 07Fh), 3; Se habilita la interrupción por cambio en PORTB
 bsf ((INTCON) and 07Fh), 5; Se habilita la interrupción por overflow de TMR0
 bcf ((INTCON) and 07Fh), 0; Se borra el valor en ((INTCON) and 07Fh), 0
 bcf ((INTCON) and 07Fh), 2; Se borra el valor en ((INTCON) and 07Fh), 2
    return
 ...
 bcf ((INTCON) and 07Fh), 0
    ...
    config_TMR0:
 BANKSEL TRISA
 bcf ((OPTION_REG) and 07Fh), 5; Se utiliza el reloj interno como reloj
 bcf ((OPTION_REG) and 07Fh), 3; El pre escalador se asigna a TIMER0
 bsf ((OPTION_REG) and 07Fh), 2
 bcf ((OPTION_REG) and 07Fh), 1
 bcf ((OPTION_REG) and 07Fh), 0; Se asignó el prescaler de 32 al timer
 res_TMR0
    return
 ...
 btfsc PORTD,7
 call res_bits_PORTD
 bcf ((INTCON) and 07Fh), 0; Se apaga la bandera de ((INTCON) and 07Fh), 0
    return
